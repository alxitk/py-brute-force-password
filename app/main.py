...
def get_range(start: int, end: int) -> None:
...
def brute_force_password() -> None:
    passwords = []
    max_num = 100000000
    chunk_size = max_num // 4

    for i in range(4):
        start = i * chunk_size
        end = start + chunk_size if i < (4 - 1) else max_num

        process = multiprocessing.Process(target=get_range, args=(start, end))
